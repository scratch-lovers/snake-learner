...
from tf_boardTEST import TFBoard
...
class LearningEnvironment(py_environment.PyEnvironment):
    ...
    def __init__(self):
        ...
        self._observation_spec = array_spec.BoundedArraySpec(
            ...
            maximum=[9, 9, 9, 9, 3], name='observation')

        self.board = TFBoard()
        first_observation = self.board.setup_board()

        snake_head_y = first_observation[0]
        snake_head_x = first_observation[1]
        apple_y = first_observation[2]
        apple_x = first_observation[3]

        self.__last_dist = int(math.sqrt((snake_head_y - apple_y) ** 2 + (snake_head_x - apple_x) ** 2))
    ...
    def calculate_dist(self):
        snake_head_y = self.__observation[0]
        snake_head_x = self.__observation[1]
        apple_y = self.__observation[2]
        apple_x = self.__observation[3]
        return int(math.sqrt((snake_head_y - apple_y) ** 2 + (snake_head_x - apple_x) ** 2))
    ...
    def _step(self, action):
        ...
        if self.__episode_ended or self.__tick == self.__max_tick:
            if self.__tick == self.__max_tick:
                return time_step.termination(self.__observation, 200)
            else:
            ...
            if isinstance(self.__board_action, ActionAddMove):
                ...
                return time_step.transition(self.__observation, reward=(1 + self.__apples_eaten) * 1000, discount=1.0)
            if self.__last_dist < self.calculate_dist():
                self.__last_dist = self.calculate_dist()
                return time_step.transition(
                    self.__observation, reward=1, discount=1.0)
            elif self.__last_dist == self.calculate_dist():
                return time_step.transition(self.__observation, reward=0, discount=1.0)
            else:
                self.__last_dist = self.calculate_dist()
                return time_step.transition(self.__observation, reward=-1, discount=1.0)
